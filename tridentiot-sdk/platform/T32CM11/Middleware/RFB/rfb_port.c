 ...
 *    LOCAL FUNCTIONS
 *************************************************************************************************/
void  reg_cal(uint32_t *reg_value,
              uint8_t  bitShift,
              uint32_t bitMask,
              uint32_t data)
{
...
void  reg_byte_invert(uint32_t reg_value,
                      uint8_t  bitShift,
                      uint32_t bitMask,
                      uint8_t  *data)
{
...
#endif /* if RFB_FIX_TX_POWER_SUPPORT */
                                    ...
                                    uint8_t          band_type)
{
    ...
    switch (modem)
    {
        ...
        case RFB_MODEM_OQPSK:
            ...
            break;
#endif /* if (defined RFB_SUBG_ENABLED && RFB_SUBG_ENABLED == 1) */
...
#endif /* if (defined RFB_ZIGBEE_ENABLED && RFB_ZIGBEE_ENABLED == 1) */

#if (defined RFB_SUBG_ENABLED && RFB_SUBG_ENABLED == 1)

RFB_EVENT_STATUS rfb_port_subg_init(rfb_interrupt_event_t *_rfb_interrupt_event,
...
#endif /* if (defined RFB_BLE_ENABLED && RFB_BLE_ENABLED == 1) */
...
RFB_EVENT_STATUS rfb_port_wake_on_radio_set(uint32_t rf_frequency,
                                            uint16_t rx_on_time,
                                            uint32_t sleep_time)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
...
RFB_EVENT_STATUS rfb_port_receive_at(uint8_t  rf_channel,
...
RFB_EVENT_STATUS rfb_port_2ch_scan_frequency_set(uint8_t  scan_enable,
                                                 uint32_t rf_freq1,
                                                 uint32_t rf_freq2)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
...
bool rfb_port_zb_is_channel_free(uint32_t rf_frequency,
...
bool rfb_port_subg_is_channel_free(uint32_t rf_frequency,
                                   uint8_t  rssi_threshold)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
    uint8_t          rssi         = 0;
...
RFB_WRITE_TXQ_STATUS rfb_port_data_send(uint8_t  *tx_data_address,
...
RFB_EVENT_STATUS rfb_port_15p4_address_filter_set(uint8_t  mac_promiscuous_mode,
...
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
    event_status                  = rfb_comm_15p4_address_filter_set(mac_promiscuous_mode,
                                                                     short_source_address,
                                                                     long_source_address_0,
                                                                     long_source_address_1,
                                                                     pan_id,
                                                                     isCoordinator);
...
/* MULTI-PAN FUNCTIONS */
void rfb_mp_address_filter_set(uint8_t  mac_promiscuous_mode,
                               uint16_t short_source_address,
                               uint32_t long_source_address_0,
                               uint32_t long_source_address_1,
                               uint16_t pan_id,
...
RFB_EVENT_STATUS rfb_port_15p4_mac_pib_set(uint32_t a_unit_backoff_period,
...
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
    event_status                  = rfb_comm_15p4_mac_pib_set(a_unit_backoff_period,
...
RFB_EVENT_STATUS rfb_port_15p4_phy_pib_set(uint16_t a_turnaround_time,
...
#endif /* if ((defined RFB_ZIGBEE_ENABLED && RFB_ZIGBEE_ENABLED == 1) || (defined RFB_15p4_MAC_ENABLED && RFB_15p4_MAC_ENABLED == 1)) */
...
RFB_EVENT_STATUS rfb_port_subg_rx_config_set(uint8_t         data_rate,
...
RFB_EVENT_STATUS rfb_port_subg_tx_config_set(tx_power_level_t tx_power,
                                             uint8_t          data_rate,
                                             uint16_t         preamble_len,
                                             fsk_mod_t        mod_idx,
                                             crc_type_t       crc_type,
...
RFB_EVENT_STATUS rfb_port_ble_modem_set(uint8_t data_rate,
...
RFB_EVENT_STATUS rfb_port_ble_mac_set(uint32_t sfd_content,
...
RFB_EVENT_STATUS rfb_port_tx_power_set(uint8_t band_type,
                                       uint8_t power_index)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
...
RFB_EVENT_STATUS rfb_port_tx_power_set_oqpsk(uint8_t band_type,
                                             uint8_t power_index)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
    uint8_t          band_type_ruci;
...
RFB_EVENT_STATUS rfb_port_fix_15dbm_tx_power_set(bool    enable,
                                                 uint8_t band_type)
{
...
RFB_EVENT_STATUS rfb_port_15p4_short_addr_ctrl(uint8_t ctrl_type,
                                               uint8_t *short_addr)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
...
RFB_EVENT_STATUS rfb_port_15p4_extend_addr_ctrl(uint8_t ctrl_type,
                                                uint8_t *extend_addr)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
...
/* MULTI-PAN FUNCTIONS */
void rfb_mp_src_addr_match_ctrl(uint8_t ctrl_type,
                                uint8_t pan_num)
{
    uint32_t temp_data;
    uint8_t  page;
...
void rfb_mp_short_addr_ctrl(uint8_t ctrl_type,
                            uint8_t *short_addr,
                            uint8_t pan_num)
{
    uint32_t temp_data;
...
void rfb_mp_extend_addr_ctrl(uint8_t ctrl_type,
                             uint8_t *extend_addr,
                             uint8_t pan_num)
{
    uint32_t temp_data;
...
void rfb_mp_key_set(uint8_t *key_addr,
                    uint8_t pan_num)
{
    uint32_t temp_data;
    uint8_t  page;
...
RFB_EVENT_STATUS rfb_port_csl_receiver_ctrl(uint8_t  csl_receiver_ctrl,
                                            uint16_t csl_period)
{
    RFB_EVENT_STATUS event_status = RFB_EVENT_SUCCESS;
...
uint8_t rfb_port_ack_packet_read(uint8_t *rx_data_address,
                                 uint8_t *rx_time_address,
                                 bool    is2bytephr)
{
    uint32_t temp_data;
    uint16_t temp_addr;
...
uint32_t rfb_port_frame_counter_get(void)
{
    ...
    frame_counter = (((frame_counter & 0xFF000000) >> 24) | ((frame_counter & 0x00FF0000) >> 8) | ((frame_counter & 0x0000FF00) <<
                                                                                                   8) | ((frame_counter & 0x000000FF) << 24));
