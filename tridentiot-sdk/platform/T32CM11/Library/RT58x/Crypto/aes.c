...
extern void wait_crypto_operation_finish(uint8_t op_num,
                                         uint8_t sb_num);
...
extern void aes_encryption(uint8_t       round,
                           const uint8_t *in_buf,
                           uint8_t       *out_buf);

extern void aes_decryption(uint8_t       round,
                           const uint8_t *in_buf,
                           uint8_t       *out_buf);
...
extern void crypto_copy(uint32_t *p_dest_addr,
                        uint32_t *p_src_addr,
                        uint32_t size);
extern void crypto_start(uint8_t op_num,
                         uint8_t sb_num);
 ...
 */
static void keyexpansion(uint8_t       *round_key,
                         const uint8_t *key,
                         uint8_t       round)
{
...
uint32_t aes_key_init(struct aes_ctx *ctx,
                      const uint8_t  *key,
                      uint32_t       keybits)
{
...
uint32_t aes_ecb_encrypt(struct aes_ctx *ctx,
                         const uint8_t  *in_buf,
                         uint8_t        *out_buf)
{
    if (ctx == NULL)
...
uint32_t aes_ecb_decrypt(struct aes_ctx *ctx,
                         const uint8_t  *in_buf,
                         uint8_t        *out_buf)
{
...
uint32_t aes_iv_set(struct aes_ctx *ctx,
                    const uint8_t  *iv)
{
    if (ctx == NULL)
    {
...
uint32_t aes_iv_get(struct aes_ctx *ctx,
                    uint8_t        *iv)
{
    if (ctx == NULL)
    {
...
void aes_xorwith(uint32_t       *result,
                 uint32_t       *buf1,
                 const uint32_t *buf2)
{
    /*
...
uint32_t aes_cbc_buffer_encrypt(struct aes_ctx *ctx,
                                uint8_t        *in_buf,
                                uint8_t        *out_buf,
                                uint32_t       length)
{
...
uint32_t aes_cbc_buffer_decrypt(struct aes_ctx *ctx,
                                uint8_t        *in_buf,
                                uint8_t        *out_buf,
                                uint32_t       length)
{
...
static void ccm_cbc_mac(struct aes_ccm_encryption_packet *ccm_packet,
                        uint8_t                          *mac_buf)
{
