...
void wait_crypto_operation_finish(uint8_t op_num,
                                  uint8_t sb_num);
...
void crypto_start(uint8_t op_num,
                  uint8_t sb_num)
{
...
#else  /* if MODULE_ENABLE(SUPPORT_MULTITASKING) */
/*
...
#endif /* if MODULE_ENABLE(SUPPORT_MULTITASKING) */
...
void aes_encryption(uint8_t       round,
...
#endif /* if  MODULE_ENABLE(CRYPTO_FreeRTOS_ISR_ENABLE) */
...
void aes_decryption(uint8_t       round,
                    const uint8_t *in_buf,
                    uint8_t       *out_buf)
{
    crypto_copy((uint32_t*)(CRYPTO_BASE + 0xEF0), (uint32_t*)in_buf, 4);
...
#endif /* if MODULE_ENABLE(CRYPTO_AES_ENABLE) */
...
void gf2m_point_b163_mult(uint32_t *p_result_x,
                          uint32_t *p_result_y,
                          uint32_t *target_x,
                          uint32_t *target_y,
                          uint32_t *target_k)
...
#endif /* if MODULE_ENABLE(CRYPTO_SECT163R2_ENABLE) */
...
uint32_t gfp_point_p192_mult(uint32_t *p_result_x,
                             uint32_t *p_result_y,
                             uint32_t *target_x,
                             uint32_t *target_y,
                             uint32_t *target_k)
...
uint32_t gfp_point_p256_mult(ECPoint_P256 *p_result_point,
...
static int vli_cmp(uint32_t *p_left,
                   uint32_t *p_right)
{
    int i;
...
void gfp_scalar_vxh_p256(uint32_t *p_result,
                         uint32_t *p_x,
                         uint32_t *p_hash,
                         uint32_t *p_v)
{
...
void gfp_scalar_modmult_p256(uint32_t *p_result,
                             uint32_t *p_x,
                             uint32_t *p_y)
{
    /*Here we load required function again..*/
...
uint32_t gfp_point_p256_invert(ECPoint_P256 *p_invert,
                               ECPoint_P256 *p_point)
{
...
#endif /* if MODULE_ENABLE(CRYPTO_SECP256R1_ENABLE) */
...
void curve25519_point_mul(uint32_t *blind_zr,
                          uint32_t *public_key,
                          uint32_t *secret_key,
                          uint32_t *base_point)
{
    ...
    uint32_t blind_zr_random_default[] =
    {
        0x2CF92B82, 0x459E8BA8, 0x0B55F9584, 0x6DB0E537,
...
#endif /* if MODULE_ENABLE(CRYPTO_C25519_ENABLE) */
